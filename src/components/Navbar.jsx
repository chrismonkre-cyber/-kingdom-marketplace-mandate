    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-black/40 border-b border-yellow-700/30">
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="hidden sm:inline font-cinzel text-sm md:text-base font-bold text-yellow-300 drop-shadow">Kingdom Marketplace</span>
        </Link>
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(l => (
            <Link key={l.to} to={l.to}
              className={`px-3 py-1.5 rounded font-cinzel text-sm transition-colors ${location.pathname === l.to ? "bg-yellow-600/30 text-yellow-200" : "text-yellow-100/80 hover:text-yellow-200 hover:bg-yellow-600/20"}`}>
              {l.label}
            </Link>
          ))}
        </div>
        ...
        <div className="md:hidden bg-black/80 backdrop-blur-lg border-t border-yellow-700/30">
          {NAV_LINKS.map(l => (
            <Link key={l.to} to={l.to} onClick={() => setOpen(false)}
              className={`block px-6 py-3 font-cinzel text-sm border-b border-yellow-900/20 ${location.pathname === l.to ? "text-yellow-300 bg-yellow-900/20" : "text-yellow-100/80"}`}>
              {l.label}
            </Link>
          ))}
        </div>
